...
for path in directories:
    data = pd.read_csv(path+"/accelData.csv") 
    df = pd.DataFrame(columns = col)
    for i in range(0,int(len(data.index)/wf+1)):
        arr_x = data.loc[i*wf:i*wf+(wf-1), 'ACC_X'].tolist()
        arr_y = data.loc[i*wf:i*wf+(wf-1), 'ACC_Y'].tolist()
        arr_z = data.loc[i*wf:i*wf+(wf-1), 'ACC_Z'].tolist()
        if len(arr_x) > 2:
        ...
        df.to_csv('dataset/accelerometer/'+path + '.csv', index = False)
